...
def run_zeroshot_evaluation(cfg: Dict):
    ...
    for mode in quant_modes:
        ...
        results = simple_evaluate(model=lm_eval_model, tasks=tasks)
        
        # 5. Extract Metrics
        mode_metrics = {"format": mode}
        for task_name, task_results in results["results"].items():
            # lm-eval usually reports both 'acc,none' and 'acc_norm,none'. 
            # We prefer acc_norm if available, else acc.
